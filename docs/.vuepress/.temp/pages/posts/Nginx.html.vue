...
<p>参考文章  https://www.jb51.net/article/148809.htm</p>
<h2 id="nginx-location-路径匹配规则" tabindex="-1"><a class="header-anchor" href="#nginx-location-路径匹配规则" aria-hidden="true">#</a> Nginx Location 路径匹配规则</h2>
<table>
<thead>
<tr>
<th>没有前缀</th>
<th>匹配以指定模式开头的location</th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td>^~</td>
<td>非正则匹配，匹配以指定模式开头的location</td>
</tr>
<tr>
<td>/</td>
...
<h2 id="location匹配顺序" tabindex="-1"><a class="header-anchor" href="#location匹配顺序" aria-hidden="true">#</a> <strong>location匹配顺序</strong></h2>
<p>多个正则location直接按书写顺序匹配，成功后就不会继续往后面匹配</p>
<p>普通（非正则）location会一直往下，直到找到匹配度最高的（最大前缀匹配）</p>
<p>当普通location与正则location同时存在，如果正则匹配成功,则不会再执行普通匹配</p>
<p>所有类型location存在时，“=”匹配 &gt; “^~”匹配 &gt; 正则匹配 &gt; 普通（最大前缀匹配）</p>
<p>示例</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>location <span class="token operator">=</span> / <span class="token punctuation">{</span>
...
<tr>
<td style="text-align:center">+</td>
<td style="text-align:center">重复1次或更多次</td>
</tr>
<tr>
<td style="text-align:center">*</td>
...
<tr>
<td style="text-align:center">$</td>
<td style="text-align:center">匹配字符串的介绍</td>
</tr>
<tr>
...
<td style="text-align:center">匹配单个字符c</td>
</tr>
<tr>
<td style="text-align:center">[a-z]</td>
<td style="text-align:center">匹配a-z小写字母的任意一个</td>
</tr>
</tbody>
</table>
<p>小括号()之间匹配的内容，可以在后面通过$1来引用，$2表示的是前面第二个()里的内容。</p>
<p><strong>正则里面容易让人困惑的是\转义特殊字符。</strong></p>
<ol>
<li>
<p>无斜杠</p>
<blockquote>
<p>proxy_pass：http://localhost:3000。</p>
<p><strong>无斜杆location匹配到的部分也属于请求的部分</strong>。</p>
<p>location无论用/get还是用/get/只要匹配上之后都会将整个请求部分/get/test加到proxy_pass上</p>
<p>http://localhost:3000+/get/test等于请求http://localhost:3000/get/test</p>
</blockquote>
</li>
<li>
<p>有斜杠</p>
...
<blockquote>
<p>proxy_pass：http://localhost:3000/。</p>
<p>有斜杆location匹配到的部分只用于匹配，不属于请求部分，需要在请求部分将location匹配到的部分剔除。</p>
<p>location用/get则是http://localhost:3000/+(/get/test -/get)等于请求http://localhost:3000//test</p>
<p>location用/get/则是http://localhost:3000/+(/get/test -/get/)等于请求http://localhost:3000/test</p>
</blockquote>
<ol start="3">
<li>
<p>有斜杠也有字符串</p>
<blockquote>
<p>proxy_pass：http://localhost:3000/abc。</p>
<p>同有斜杆的规则，在请求部分剔除location后加在上面即可。</p>
<p>location用/get则是http://localhost:3000/abc+(/get/test -/get)等于请求http://localhost:3000/abc/test</p>
<p>location用/get/则是http://localhost:3000/abc+(/get/test -/get/)等于请求http://localhost:3000/abctest</p>
</blockquote>
<p>location /resources {
...
<p>http://10.20.1.60:18890/galaxy/resources/</p>
</li>
</ol>
</div></template>
